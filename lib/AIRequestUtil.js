...
function getQuestionInfo (type, key) {
  ...
  let questionWidget = selector().className('android.widget.TextView').filter(v => {
    let b = v.bounds()
    return b.left < 200 && b.top > config.scaleRate * 300 && b.top < config.scaleRate * 1120
  }).findOne(2000)
  let answerWidgets = null
  let t = threads.start(function () {
    let depth = 18
    if (questionWidget) {
      depth = questionWidget.depth() + 1
    }
    answerWidgets = selector().className('android.widget.TextView').depth(depth).untilFind()
  })
  t.join(5000)
  t.interrupt()
  ...
  let answers = []
  let answerPosition = []
  for (let i = 0; i < answerWidgets.length && i < 2; i++) {
